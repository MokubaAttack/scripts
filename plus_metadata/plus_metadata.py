...
keys=[
    "input","pr","ne","st","sa","cf","se","cl","ckpt","lora1","lora2","lora3","lora4","embed1","embed2","embed3","embed4","w1","w2","w3","w4","vae","hu","hs","hum","ds","pag","sc"
]

def plus(path,pr,ne,ckpt,st="",sa="",sc="",cf="",se="",cl="",hu="",hs="",hum="",ds="",pag="",loras=[],ws=[],embeds=[],vae="",win=None):
    if win!=None:
...
def run(vs,win):
    ...
    for path in paths:
        c=plus(path,vs["pr"],vs["ne"],vs["ckpt"],vs["st"],vs["sa"],vs["sc"],vs["cf"],vs["se"],vs["cl"],vs["hu"],vs["hs"],vs["hum"],vs["ds"],vs["pag"],loras,ws,embeds,vs["vae"],win)
