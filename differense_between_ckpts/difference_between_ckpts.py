...
def diff_ckpt(
	...
	c=[unet_out,text1_out,text2_out]
	...
	while True:
		...
		if event == sg.WINDOW_CLOSED:
			...
			if values["out"]!="" and values["ckpt1"]!="" and values["ckpt2"]!="":
				...
				unet_out=values["unet"]
				text1_out=values["text1"]
				text2_out=values["text2"]
				
				thread1 = threading.Thread(target=diff_ckpt,args=(paths,out_path,dim,unet_out,text1_out,text2_out,window))
				thread1.start()
		elif "-copy-" in event:
			try:
