    ...
    try:
        ...
        if not(os.path.exists(base_safe)):
            if win==None:
                ...
                notification.notify(title="error",message="the ckpt file doesn't exist.")
            return
        ...
        if vae_safe!="":
            if os.path.exists(vae_safe):
                ...
                if win==None:
                    ...
                    notification.notify(title="error",message="the vae file doesn't exist.")
                return
        ...
        if lora1!="":
            if os.path.exists(lora1):
                ...
                if win==None:
                    print("error : the lora1 file doesn't exist.")
                else:
                    win['RUN'].Update(disabled=False)
                    win["info"].update("error : the lora1 file doesn't exist.")
                    notification.notify(title="error",message="the lora1 file doesn't exist.")
                return
        ...
        if lora2!="":
            if os.path.exists(lora2):
                ...
                if win==None:
                    print("error : the lora2 file doesn't exist.")
                else:
                    win['RUN'].Update(disabled=False)
                    win["info"].update("error : the lora2 file doesn't exist.")
                    notification.notify(title="error",message="the lora2 file doesn't exist.")
                return
        ...
        if lora3!="":
            if lora3!="" and os.path.exists(lora3):
                ...
                if win==None:
                    print("error : the lora3 file doesn't exist.")
                else:
                    win['RUN'].Update(disabled=False)
                    win["info"].update("error : the lora3 file doesn't exist.")
                    notification.notify(title="error",message="the lora3 file doesn't exist.")
                return
        ...
        if win==None:
            ...
            notification.notify(title="fin",message=out_safe)
    except:
        if win==None:
