...
def tile_up(
    ...
    images=[]
    if img_paths==[]:
        print("Please select image files.")
        return []
    else:
    ...
    meta_dict={}
    memo="seed\n"
    if len(seed)<len(img_paths):
        for i in range(len(img_paths)-len(seed)):
            seed.append(seed[-1])
    elif len(seed)>len(img_paths):
        seed=seed[:len(img_paths)]
    for i in range(len(seed)):
        try:
            ...
            seed[i]=random.randint(1, sys.maxsize)
        memo=memo+img_paths[i]+" : "+str(seed[i])+"\n"
    clear_output(True)
    print(memo)
    ...
    for i in range(len(seed)):
        ...
        if img_paths[i].endswith(".jpg"):
            meta_dict["input"]=img_paths[i].replace(".jpg","_up.jpg")
        else:
            meta_dict["input"]=img_paths[i].replace(".png","_up.png")
