...
def _normalize_lora_unet_path(path: str) -> str:
    ...
    #new
    placeholders = {
        "adaln_modulation_self_attn":"moku1",
        "adaln_modulation_cross_attn":"moku2",
        "adaln_modulation_mlp":"moku3",
        "self_attn":"moku4",
        "cross_attn":"moku5",
        "output_proj":"moku6",
        "q_proj":"moku7",
...
def _collect_lora_module_payloads(
    ...
    for original_key, value in state_dict.items():
        ...
        module_path, suffix = _split_lora_key_suffix(key)
        mapped_path = _map_anima_module_path(module_path)
        if not(mapped_path in payloads):
            payloads[mapped_path]={}

        if suffix == "alpha":
            payloads[mapped_path][suffix] = _to_float_alpha(value)
        else:
            payloads[mapped_path][suffix] = value
