...
def mergeckpt(ckpts,weights,v,out_path,win=None):
    ...
    try:
        ...
        for k in data_dict:
            ...
            out_dict={}
            out_dict[k]=torch.zeros(data_dict[k],dtype=torch.float32)
            for i in range(2):
                    ...
                    if k.startswith("model.diffusion_model.middle_block."):
                        ...
                        if k.startswith("first_stage_model."):
                            if v!=-1:
                                ...
                                out_dict[k]=out_dict[k]+(t1*weights[0][i])
                        else:
                            out_dict[k]=out_dict[k]+(t1*weights[0][i])
                    del state_dict,t1
...
if __name__=="__main__":
    ...
    layout=[
        ...
        [sg.Text("weight of ckpt2"),sg.Input(key="w0",right_click_menu=grp_rclick_menu["w0"], size=(20, 1))],
        [
        ...
        [
            ...
            sg.Text("Dropout probability",key="dpt"), sg.Input(key="dp",size=(10, 1),right_click_menu=grp_rclick_menu["dp"]),
            sg.Text("seed",key="seedt"), sg.Input(key="seed",right_click_menu=grp_rclick_menu["seed"])
        ],
    ...
    for i in range(9):
        window["i"+str(i)].update(visible=False)
        window["o"+str(i)].update(visible=False)
    window["dpt"].update(visible=False)
    window["dp"].update(visible=False)
    window["seedt"].update(visible=False)
    window["seed"].update(visible=False)
    ...
    while True:
        ...
        if event == sg.WINDOW_CLOSED:
            ...
            try:
                window["w0"].update(visible=not(values["block"]))
                window["base"].update(visible=values["block"])
                window["mid"].update(visible=values["block"])
                for i in range(9):
                    window["i"+str(i)].update(visible=values["block"])
                    window["o"+str(i)].update(visible=values["block"])
            except:
                pass
