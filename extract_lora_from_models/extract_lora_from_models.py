...
def diff_ckpt(paths,out_path,dim,c,win):
    ...
    try:
        ...
        for name in names:
            ...
            sd[name + ".lora_up.weight"] = up_weight.to("cpu").contiguous()
            sd[name + ".lora_down.weight"] = down_weight.to("cpu").contiguous()
            sd[name + ".alpha"] = torch.tensor(module_new_rank, device="cpu")
